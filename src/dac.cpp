...
DAC::DAC(std::string spiPath, std::string gpiochipName, std::uint8_t rstPin,
  ...
  if (res < 0) {
    logs::log(ERR, "Failed to configure dac81408: spidev[%s], gpiochip[%s]\n",
...
int DAC::setChannelVoltage(std::uint8_t ch, double outVoltage) {
  ...
  if (outVoltage > max) {
    logs::log(ERR, "Invalid set point for range! set_point[%.2f], max[%.2f]\n",
              outVoltage, max);
    return -1;
  }

  std::uint16_t val = static_cast<std::uint16_t>(outVoltage / max * UINT16_MAX);
