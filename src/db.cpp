...
int SqliteDb::createRegisteredSensorsTable(void) {
    ...
    logs::log(INFO, "registered table was created (if it didn't exist)!\n");
    return 0;
}
...
int SqliteDb::addRegisteredSensor(std::string const &name,
    ...
    if (sqlite3_prepare_v2(dbHandle_, stmtStr, -1, &stmt, nullptr) !=
        SQLITE_OK) {
...
SqliteDb::getRegisteredSensors() {
    ...
    const char *stmtStr = "SELECT sensorName, sensorType FROM registered;";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(dbHandle_, stmtStr, -1, &stmt, nullptr) !=
        SQLITE_OK) {
...
int SqliteDb::addTrackedSensor(std::string const &name,
    ...
    const char *stmtStr = "INSERT OR IGNORE INTO tracked (sensorName, channel) "
                          "VALUES (?, ?);";
