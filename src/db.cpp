...
int SqliteDb::prepareStmt(void) {
  ...
  const char* measStmt =
      "INSERT INTO measurements (sensorName, channel, "
      "value, timestamp_ms) VALUES (?, ?, ?, ?);";

  const char* actuationStmt =
      "INSERT INTO actuation (dacName, channel, "
...
int SqliteDb::rollback(void) {
  if (!dbOpen_) return -1;
  char* errMsg = nullptr;
  if (sqlite3_exec(dbHandle_, "ROLLBACK;", nullptr, nullptr, &errMsg) !=
      SQLITE_OK) {
    logs::log(ERR, "ROLLBACK failed: %s\n", errMsg ? errMsg : "");
    sqlite3_free(errMsg);
    return -1;
  }
  return 0;
...
int SqliteDb::addActuation(ActuationEntry& entry) {
  if (!dbOpen_ || !insertActuationStmt_) return -1;
